...
import { CODE_MODEL_TIERS, CodeModelTier } from '../../config/codeModelTiers';
...
export const ClaudeAssistantPanel: React.FC<ClaudeAssistantPanelProps> = ({
  ...
  const [selectedModelTier, setSelectedModelTier] = useState<CodeModelTier>(() => {
    try {
      const saved = localStorage.getItem('claude_code_model_tier');
      if (saved && ['fable', 'opus', 'sonnet', 'auto'].includes(saved)) {
        return saved as CodeModelTier;
      }
    } catch {}
    return 'opus';
  });
  ...
  const handleSelectTier = (tier: CodeModelTier) => {
    setSelectedModelTier(tier);
    try {
      localStorage.setItem('claude_code_model_tier', tier);
    } catch {}
    setIsModelDropdownOpen(false);
  };
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative" ref={modelDropdownRef}>
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs text-[#ECEBE7] hover:text-white px-2.5 py-1 rounded-lg bg-[#242320] hover:bg-[#2B2A27] border border-[#33322E] transition-all cursor-pointer"
                        title="Select coding model tier"
                      >
                        <span className="font-medium text-[11px]">
                          {CODE_MODEL_TIERS.find((t) => t.id === selectedModelTier)?.displayName || 'Opus'}
                        </span>
                        <ChevronDown className={`w-3 h-3 text-[#8C8A82] shrink-0 transition-transform ${isModelDropdownOpen ? 'rotate-180' : ''}`} />
                      </button>

                      {isModelDropdownOpen && (
                        <div className="absolute right-0 bottom-full mb-2 w-72 bg-[#1C1B19] border border-[#2B2A27] rounded-2xl shadow-2xl p-2 z-50 text-xs">
                          <div className="px-2 py-1 text-[10px] text-[#706E68] font-semibold uppercase tracking-wider border-b border-[#262522] mb-1.5">
                            Coding Model Tier
                          </div>
                          <div className="space-y-1">
                            {CODE_MODEL_TIERS.map((tier) => {
                              const isSelected = selectedModelTier === tier.id;
                              return (
                                <button
                                  key={tier.id}
                                  type="button"
                                  onClick={() => handleSelectTier(tier.id)}
                                  className={`w-full text-left p-2 rounded-xl flex items-start justify-between transition-colors cursor-pointer ${
                                    isSelected
                                      ? 'bg-[#262522] border border-[#3A3833]'
                                      : 'hover:bg-[#22211E] border border-transparent'
                                  }`}
                                >
                                  <div className="flex-1 pr-2">
                                    <div className="flex items-center gap-1.5">
                                      <span className={`text-xs font-semibold ${isSelected ? 'text-[#ECEBE7]' : 'text-[#D4D3CE]'}`}>
                                        {tier.displayName}
                                      </span>
                                      {tier.badge && (
                                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#DA7756]/20 text-[#DA7756] font-medium">
                                          {tier.badge}
                                        </span>
                                      )}
                                    </div>
                                    <div className="text-[11px] text-[#A8A69E] font-medium mt-0.5 leading-tight">
                                      {tier.tagline}
                                    </div>
                                    <div className="text-[10px] text-[#706E68] leading-tight mt-1">
                                      {tier.description}
                                    </div>
                                  </div>
                                  {isSelected && (
                                    <Check className="w-3.5 h-3.5 text-[#DA7756] shrink-0 mt-0.5" />
                                  )}
                                </button>
                              );
                            })}
                          ...
                          <div className="mt-2 pt-1.5 border-t border-[#262522]">
                            <button
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                                setShowAdvancedModels(!showAdvancedModels);
                              }}
                              className="w-full flex items-center justify-between px-2 py-1 text-[10px] text-[#706E68] hover:text-[#A8A69E] transition-colors cursor-pointer"
                            >
                              <span>Advanced details</span>
                              <ChevronDown className={`w-3 h-3 transition-transform ${showAdvancedModels ? 'rotate-180' : ''}`} />
                            </button>
                            {showAdvancedModels && (
                              <div className="mt-1 p-2 bg-[#141413] rounded-lg border border-[#262522] space-y-1 text-[10px] text-[#8C8A82]">
                                {CODE_MODEL_TIERS.filter((t) => t.backendDefault).map((t) => (
                                  <div key={t.id} className="flex items-center justify-between gap-2">
                                    <span className="font-medium text-[#ECEBE7]">{t.displayName}:</span>
                                    <span className="font-mono text-[9px] text-[#706E68] truncate max-w-[140px]" title={t.backendDefault}>
                                      {t.backendDefault}
                                    </span>
                                  </div>
                                ))}
                                <div className="text-[9px] text-[#555] pt-1 border-t border-[#201F1D]">
                                  Overrides via FABLE_MODEL, OPUS_MODEL, SONNET_MODEL env vars.
                                </div>
                              </div>
                            )}
                          </div>
